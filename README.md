...
- **Single Project (Monorepo):** Vercel builds frontend/package.json (dist) + api/index.ts (@vercel/node), routes /api/* -> backend, /health -> backend, /assets/* -> frontend, /* -> frontend. Env: VITE_API_URL=/ (same origin, no CORS)
